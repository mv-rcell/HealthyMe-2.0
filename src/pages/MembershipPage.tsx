...
const MembershipPage = () => {
    ...
    const fetchClientProfiles = async () => {
      try {
        ...
        const groupedProfiles = {
          starter: profiles.filter(
            (profile) => profile.membership_tier?.toLowerCase() === "starter"
          ),
          pro: profiles.filter(
            (profile) => profile.membership_tier?.toLowerCase() === "pro"
          ),
          elite: profiles.filter(
            (profile) => profile.membership_tier?.toLowerCase() === "elite"
          ),
        };
  ...
  const handlePesaPalPayment = async () => {
    ...
    try {
      ...
      console.log("PesaPal payment response:", { data, error });
                  ...
                  >
                    ...
                    <div className="mt-8 pt-8 border-t border-gray-100 dark:border-gray-800">
                      ...
                      {isLoading ? (
                        ...
                        <div className="space-y-3">
                          {clientProfiles[
                            plan.id.toLowerCase() as keyof typeof clientProfiles
                          ].length > 0 ? (
                            clientProfiles[
                              plan.id.toLowerCase() as keyof typeof clientProfiles
                            ].map((profile) => (
              ...
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  ...
                  <DialogDescription className="text-center">
                    Complete your payment via PesaPal to activate your{" "}
                    {selectedPlan?.name} membership. Supports M-Pesa, Airtel
                    Money, Visa, and Mastercard.
                  </DialogDescription>
                </DialogHeader>

                <div className="text-center p-4 bg-primary/5 rounded-lg my-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
                {Object.entries(clientProfiles).map(([tier, profiles]) => (
                  <div key={tier} className="space-y-6">
                    <h3 className="text-xl font-bold text-center capitalize mb-6">
                      {tier} Members
                    </h3>
                    {isLoading ? (
                        ...
                        {profiles.length > 0 ? (
                          profiles.map((profile) => (
                            ...
                            >
                              <AspectRatio
                                ratio={1 / 1}
                                className="mb-3 relative overflow-hidden rounded-md bg-muted"
                              >
